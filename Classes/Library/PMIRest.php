...
class PMIRest {
    ...
    public function __construct($option) {
        ...
        /*if (isset($GLOBALS['TSFE']->tmpl->setup['plugin.']['tx_libconnect.']['pubmanurl'])) {
            $this->host_name = substr(strstr($GLOBALS['TSFE']->tmpl->setup['plugin.']['tx_libconnect.']['pubmanurl'], "://"), 3);
        } else {*/
            //$this->host_name = substr(strstr($this->options['source_url'], "://"), 3);
        //}
        //PMILog::debug("Hostname is \"" . $this->host_name . "\"");
    }
    ...
    public function getData($uri, $query) {
        ...
        if (!$socket) {
            ...
            while (!feof($socket)) {
                $line = fgets($socket);
                if (stripos($line, 'Connection: close') !== FALSE)
                    break;
                else if (stripos($line, 'Content-Length:') !== FALSE) {
                    list(, $data_size) = explode(':', $line);
                } else if (stripos($line, 'HTTP/1.1') !== FALSE) {
                    list($protocol, $code, $status) = explode(' ', $line);
                    if ($code != 200) {
                        PMILog::push("Got HTTP Code " . $code . " (" . $this->getStatusCodeMessage($code) . ")");
                        return $code;
                    }
                }
            }
    ...
    /**
     * This class parses XML from PubMan and creates a array with information for blog posts
     * @param $data XML snippet from PubMan REST interface
     * @return array with data for blog post creation
     */
                    ...
                    if ($license == "") {
                        ...
                        if ($mdRecordsNode) {
                            ...
                            if ($license != "") {
                                ...
                                array_push($posts['log'], "Found license $license");
                            } else {
                                PMILog::debug("No CC license found");
                                array_push($posts['log'], "No CC license found");
                            }
                        }
                    }
                }
